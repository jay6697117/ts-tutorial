...
// 1.类型声明和类型断言的核心区别，就知道了类型声明是比类型断言更加严格的。
// 2.所以为了增加代码的质量，我们最好优先使用类型声明，这也比类型断言的 as 语法更加优雅。
// 3.TypeScript 是结构类型系统，类型之间的对比只会比较它们最终的结构，而会忽略它们定义时的关系

// 9.类型断言 vs 泛型

/* const windowTemp = { cache: { tom: { name: '111', run: () => console.log('run tom') } } };

function getCacheData(key: string): any {
  return (windowTemp as any).cache[key];
}
interface Cat {
  name: string;
  run(): void;
}
const tom = getCacheData('tom') as Cat;//tom: any => tom: Cat
console.log('tom', tom);
tom.run(); */

/* function getCacheData(key: string): any {
  return ((windowTemp as any).cache as any)[key];
}
interface Cat {
  name: string;
  run(): void;
...
const windowTemp = { cache: { tom: { name: '111', run: () => console.log('run tom') } } };
console.log('windowTemp', windowTemp)
const tom: Cat = getCacheData('tom'); //tom: Cat
console.log('tom', tom);
tom.run();
...
(window as any).cache = { tom: { name: '111', run: () => console.log('run tom') } };
...
(window as any).cache = { tom: { name: 'tom green', run: () => console.log('run tom ...') } };
